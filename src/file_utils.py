...
def generate_filename(params):
    ...
    if "starttime" in params:
        parts.append(
            f"start_{params['starttime'].replace('-', '').replace(':', '').replace('T', '_')}"
        )
    if "endtime" in params:
        parts.append(
            f"end_{params['endtime'].replace('-', '').replace(':', '').replace('T', '_')}"
        )
    if "minmagnitude" in params:
        parts.append(f"minmag_{params['minmagnitude']}")

    filename = (
        f"earthquakes_{'_'.join(parts)}_{timestamp}.{params.get('format', 'geojson')}"
    )
